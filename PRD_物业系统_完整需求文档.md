...
### 开发进度 (2025-03-11)
- **PC 端实施技术栈**：Next.js 16 + React 19 + Tailwind CSS 4 + Prisma
- **数据库**：SQLite（本地开发）/ MySQL（生产上线）
- **已完成**：登录认证、首页数据看板、楼宇管理、项目管理、房源管理、租客管理、部门/角色/员工管理、收费管理、账单规则、工单管理、巡检计划/任务、公告、卫生吐槽、NFC标签、设备台账等
- **代码位置**：`PMS/pms-web/`

### 小程序开发进度 (2025-03-11)
- **租客端** (pms-tenant)：uni-app Vue3 + TypeScript，登录、首页、账单、报修、公告、吐槽、个人中心
...
- [三、功能需求](#三功能需求)
  - [3.1 PC端功能](#31-pc端功能)
  - [3.2 租客端小程序功能](#32-租客端小程序功能)
  - [3.3 员工端小程序功能](#33-员工端小程序功能)
- [四、非功能需求](#四非功能需求)
- [五、数据模型](#五数据模型)
- [六、接口设计](#六接口设计)
- [七、附录](#七附录)
...
### 2.1 技术栈
前端技术栈：React + TypeScript + Ant Design（简洁现代风格，支持暗色模式，响应式设计）
小程序技术栈：微信原生小程序（简洁风格）
后端技术栈：Node.js + Express/Koa + TypeScript
数据库：MySQL 8.0+
对象存储：本地服务器文件存储
...
**功能列表：**
- 提交报事报修：选择报修类型、描述问题、上传图片、选择位置
- 工单列表：查看我的工单列表，按照不同状态区分table列表展示，点击可进入工单详情，支持查看我提交的
- 工单详情：查看工单信息进入详情页面
- 工单评价：对完成的工单进行评价（1-5星+评价内容）
...
// Part 5/5 - 租客端小程序（继续）和员工端小程序功能**3.2.5 卫生吐槽**
...
| 租客员工 | 租客企业下的员工，使用租客端小程序 |
| 租客管理员 | 租客员工中具有管理员权限的员工 |
| 物业员工 | 物业公司的工作人员，使用员工端小程序 |
| 组长 | 物业员工中负责派单的人员 |
| 账单 | 向租客收取费用的单据 |
| 工单 | 报事报修或巡检异常生成的工作单据 |
| 巡检 | 定期检查物业设施设备的工作 |
