import { NextRequest, NextResponse } from 'next/server'
import { prisma } from '@/lib/prisma'
import { getAuthUser } from '@/lib/auth'
import { z } from 'zod'

const updateSchema = z.object({
  name: z.string().min(1, '部门名称必填'),
  parentId: z.number().nullable().optional(),
  managerId: z.number().nullable().optional(),
  projectIds: z.array(z.number()).optional().default([]),
  buildingIds: z.array(z.number()).optional().default([]),
})

export async function PUT(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const user = await getAuthUser()
    if (!user) {
      return NextResponse.json({ success: false, message: '未登录' }, { status: 401 })
    }
    if (user.companyId === 0) {
      return NextResponse.json(
        { success: false, message: '超级管理员请使用员工账号登录后操作' },
        { status: 403 }
      )
    }
    const { id } = await params
    const deptId = parseInt(id, 10)
    if (isNaN(deptId)) {
      return NextResponse.json({ success: false, message: '无效的部门ID' }, { status: 400 })
    }
    const existing = await prisma.department.findFirst({
      where: { id: deptId, companyId: user.companyId },
    })
    if (!existing) {
      return NextResponse.json({ success: false, message: '部门不存在' }, { status: 404 })
    }
    const body = await request.json()
    const parsed = updateSchema.parse(body)
    if (parsed.parentId === deptId) {
      return NextResponse.json(
        { success: false, message: '上级部门不能选择自身' },
        { status: 400 }
      )
    }
    const dept = await prisma.department.update({
      where: { id: deptId },
      data: {
        name: parsed.name,
        parentId: parsed.parentId ?? null,
        managerId: parsed.managerId ?? null,
        projectIds: JSON.stringify(parsed.projectIds ?? []),
        buildingIds: JSON.stringify(parsed.buildingIds ?? []),
      },
    })
    return NextResponse.json({ success: true, data: dept })
  } catch (e) {
    if (e instanceof z.ZodError) {
      return NextResponse.json(
        { success: false, message: '参数错误', errors: e.errors },
        { status: 400 }
      )
    }
    console.error(e)
    return NextResponse.json({ success: false, message: '服务器错误' }, { status: 500 })
  }
}

export async function DELETE(
  _request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const user = await getAuthUser()
    if (!user) {
      return NextResponse.json({ success: false, message: '未登录' }, { status: 401 })
    }
    if (user.companyId === 0) {
      return NextResponse.json(
        { success: false, message: '超级管理员请使用员工账号登录后操作' },
        { status: 403 }
      )
    }
    const { id } = await params
    const deptId = parseInt(id, 10)
    if (isNaN(deptId)) {
      return NextResponse.json({ success: false, message: '无效的部门ID' }, { status: 400 })
    }
    const existing = await prisma.department.findFirst({
      where: { id: deptId, companyId: user.companyId },
      include: { _count: { select: { children: true, employees: true } } },
    })
    if (!existing) {
      return NextResponse.json({ success: false, message: '部门不存在' }, { status: 404 })
    }
    if (existing._count.children > 0) {
      return NextResponse.json(
        { success: false, message: '存在子部门，请先删除或移出子部门' },
        { status: 400 }
      )
    }
    if (existing._count.employees > 0) {
      return NextResponse.json(
        { success: false, message: '部门下存在员工，请先移出员工' },
        { status: 400 }
      )
    }
    await prisma.department.delete({ where: { id: deptId } })
    return NextResponse.json({ success: true })
  } catch (e) {
    console.error(e)
    return NextResponse.json({ success: false, message: '服务器错误' }, { status: 500 })
  }
}
