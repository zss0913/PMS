...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const b of bills) {
      ...
      const remain = paid - issued
      if (remain <= 0.0001) continue
      withAmount.push({ ...b, receiptLineAmount: remain })
    }

    if (withAmount.length === 0) {
      return NextResponse.json(
        { success: false, message: '所选账单均已足额开具收据（已缴金额已全部计入收据），无法再次生成' },
        { status: 400 }
      )
    }
    ...
    for (const tenantId of tenantIds.sort((a, b) => a - b)) {
      ...
      const billList = group
        .map(
          (b) =>
            `${b.code}　${b.feeType}　账期 ${b.period}　本次收据 ¥${b.receiptLineAmount.toFixed(2)}`
        )
        .join('\n')
