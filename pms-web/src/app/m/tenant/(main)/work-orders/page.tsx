      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {list.map((w) => (
            <li key={w.id}>
              <Link
                href={`/m/tenant/work-orders/${w.id}`}
                className="block rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 active:bg-slate-50 dark:active:bg-slate-800"
              >
                <div className="flex justify-between gap-2">
                  <span className="font-mono text-xs text-slate-500">{w.code}</span>
                  <span className="text-xs text-blue-600 dark:text-blue-400 shrink-0">{w.status}</span>
                </div>
