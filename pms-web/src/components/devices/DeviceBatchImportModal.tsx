...
function downloadFailedExcel(failedRows: FailedRow[]) {
  ...
  XLSX.writeFile(wb, '设备导入失败数据.xlsx')
}

export function DeviceBatchImportModal({
  buildings,
  onClose,
  onSuccess,
}: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const devices = rows.map((r) => ({
        ...
        maintenanceContact: r.maintenanceContact || undefined,
      ...
      const res = await fetch('/api/devices/batch', {
        ...
        credentials: 'include',
        body: JSON.stringify({ buildingId, devices }),
      })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <form id="device-batch-import-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 space-y-4">
          {error && (
                      ...
                      <tbody>
                        {result.failedRows.map((r, i) => (
                          <tr key={i} className="border-t border-slate-200 dark:border-slate-600">
                            ...
                            <td className="p-2">{r.maintenanceContact}</td>
            ...
            <>
              <div>
                <label className="block text-sm font-medium mb-1">
                  所属楼宇 <span className="text-red-500">*</span>
                </label>
                <select
                  value={buildingId}
              ...
              <div>
                ...
                {!fileName ? (
                  <label
                    htmlFor="device-batch-import-file"
                  ...
                  >
                    ...
                    <span className="text-sm text-slate-600 dark:text-slate-400">点击选择 Excel 文件</span>
                  </label>
                ) : (
